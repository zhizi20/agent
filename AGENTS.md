...
│   ├── lib/                # 工具库
│   │   ├── utils.ts        # 通用工具函数 (cn)
│   │   ├── desensitize.ts  # 敏感信息脱敏模块
│   │   └── sensitive-filter.ts # 违规内容审核模块
│   └── server.ts           # 自定义服务端入口
...
### 敏感信息脱敏（展示层）
- 模块：`src/lib/desensitize.ts`
- 触发时机：API 返回心声列表时自动脱敏，原始数据不做修改
- 脱敏范围：
  ...
  - 微信号/QQ号：关键词+内容 → 关键词+***
  - 地址信息：北京市朝阳区xxx → 北京市朝***
  - 薪资/奖金金额：月薪8000元 → 月薪****元
  - 合同/项目编号：关键词+编号 → 关键词+***
  - 客户/供应商名称：关键词+公司名 → 关键词+前2字+***
  - 工号：关键词+编号 → 关键词+***
  - 作者姓名：张三 → 张*

### 违规内容审核（发布层）
- 模块：`src/lib/sensitive-filter.ts`
- 触发时机：POST /api/voices 创建心声时
- 拦截类型：
  - 人身攻击/辱骂/歧视
  - 暴力威胁/恐吓/报复
  - 自伤/自杀风险（返回心理援助热线）
...
- 初始化：若 data/voices.json 不存在，自动创建空数组文件，不再使用种子数据初始化
- data/ 目录已加入 .gitignore，不纳入版本控制

## 批量导入

### 去重机制
- 基于 Jaccard 相似度算法检测重复反馈（阈值 0.7）
- 与数据库已有内容比对 + 本次批量内互相比对
- 管理层不执行去重
- 豁免场景：主旨明显变更（分类不同且相似度<0.5），或间隔 30 天以上且有大量新描述

### 并发处理
- 每批 40 条，最多并发 3 批
- 使用 Promise.allSettled 确保一批失败不影响其他批次
- 解析失败时自动降级为默认分类（其他/低紧急/综合管理部）
